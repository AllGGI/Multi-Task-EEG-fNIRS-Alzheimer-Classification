...
def trigger_named_eeg(act_name, trig_file, eeg_mat, fnirs_mat):
    ...
    '''
    initial_start_idx = targ_nontarg_idx[0] # 15000
    sliced_eeg_data, sliced_fnirs_data = [], {'Hb':{}, 'HbO':{}, 'THb':{}}

    ###############################################################
    # EEG Baseline correction (- mean of initial 30 sec)
    if act_name == 'V': # for 6 baseline corrections
        eeg_data = eeg_mat['data']

    else: # only one baseline correction
        eeg_data = eeg_mat['data']
        baseline_correction = np.mean(eeg_data[:initial_start_idx,:], axis=0) # (32,)
        eeg_data = eeg_data - baseline_correction

    # FNIRS Baseline correction (- initial value)
    hb_fnirs_data = fnirs_mat['Hb'] # - (fnirs_mat['Hb'][0,:])
    hbo_fnirs_data = fnirs_mat['HbO'] # - (fnirs_mat['HbO'][0,:])
    thb_fnirs_data = fnirs_mat['THb'] # - (fnirs_mat['THb'][0,:])
    ###############################################################

    if act_name == 'V': # substract just before 30 sec mean

        eeg_start_idx = initial_start_idx # first task of total 6 tasks
        fnirs_start_idx = int((eeg_start_idx / 500) * 8) # ex) 15000/500 * 8 = 240

        # fnirs (0-30sec:before task | 30-60sec:task ing | 60-90sec: after task)
        hb_before = hb_fnirs_data[0:fnirs_start_idx, :] # 0~30sec
        hb_ing = hb_fnirs_data[fnirs_start_idx:fnirs_start_idx+240, :] # 30sec~60sec
        hb_after = hb_fnirs_data[fnirs_start_idx+240:fnirs_start_idx+480, :] # 60sec~90sec
        hb_dict = {'before':hb_before, 'ing':hb_ing, 'after':hb_after}
        
        hbo_before = hbo_fnirs_data[0:fnirs_start_idx, :] # 0~30sec
        hbo_ing = hbo_fnirs_data[fnirs_start_idx:fnirs_start_idx+240, :] # 30sec~60sec
        hbo_after = hbo_fnirs_data[fnirs_start_idx+240:fnirs_start_idx+480, :] # 60sec~90sec
        hbo_dict = {'before':hbo_before, 'ing':hbo_ing, 'after':hbo_after}

        thb_before = thb_fnirs_data[0:fnirs_start_idx, :] # 0~30sec
        thb_ing = thb_fnirs_data[fnirs_start_idx:fnirs_start_idx+240, :] # 30sec~60sec
        thb_after = thb_fnirs_data[fnirs_start_idx+240:fnirs_start_idx+480, :] # 60sec~90sec
        thb_dict = {'before':thb_before, 'ing':thb_ing, 'after':thb_after}

        sliced_fnirs_data['Hb'] = hb_dict
        sliced_fnirs_data['HbO'] = hbo_dict
        sliced_fnirs_data['THb'] = thb_dict

    else:
        eeg_start_idx = initial_start_idx # first task of total 6 tasks
        fnirs_start_idx = int((eeg_start_idx / 500) * 8) # ex) 15000/500 * 8 = 240

        # fnirs (0-30sec:before task | 30 ~ -30sec:task ing | -30 ~ : after task)
        hb_before = hb_fnirs_data[0:fnirs_start_idx, :] # 0~30sec
        hb_ing = hb_fnirs_data[fnirs_start_idx:-240, :] # 30 ~ -30sec
        hb_after = hb_fnirs_data[-240:, :] # -30sec ~
        hb_dict = {'before':hb_before, 'ing':hb_ing, 'after':hb_after}
        
        hbo_before = hbo_fnirs_data[0:fnirs_start_idx, :] # 0~30sec
        hbo_ing = hbo_fnirs_data[fnirs_start_idx:-240, :] # 30 ~ -30sec
        hbo_after = hbo_fnirs_data[-240:, :] # -30sec ~
        hbo_dict = {'before':hbo_before, 'ing':hbo_ing, 'after':hbo_after}

        thb_before = thb_fnirs_data[0:fnirs_start_idx, :] # 0~30sec
        thb_ing = thb_fnirs_data[fnirs_start_idx:-240, :] # 30 ~ -30sec
        thb_after = thb_fnirs_data[-240:, :] # -30sec ~
        thb_dict = {'before':thb_before, 'ing':thb_ing, 'after':thb_after}
...
def make_sliced_data_pkl(pths, root_path, trig_list):
    ...
    for i, d_list in tqdm(enumerate(data_list)): # AD -> Normal -> NP -> MCI order
        ...
        for fn in eeg_file_list: # Patients
            ...
            all_len = [len(i) for i in fnirs_act_datalist]
            # print(all_len)
    ...
    save_file_name = 'original_fnirs_before_ing_after'
...
def segmentation_timedomain_base():
    ...
    trig_list = dict()
    trig_list['AD'] = trig_list2['AD'] + trig_list3['AD']
    trig_list['NORMAL'] = trig_list2['NORMAL'] + trig_list3['NORMAL']
    trig_list['NP'] = trig_list2['NP'] + trig_list3['NP']
    trig_list['MCI'] = trig_list2['MCI'] + trig_list3['MCI']
