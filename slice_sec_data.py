...
def trigger_named_eeg(act_name, trig_file, eeg_mat, fnirs_mat):
    ...
    for eeg_start_idx in targ_idx:
        # eeg
        eeg_sample = eeg_mat['data'][eeg_start_idx:eeg_start_idx+(eeg_sampling_rate*global_slice_sec), :] # (2500, 32)
        sliced_eeg_data.append(eeg_sample)

        # fnirs
        fnirs_start_idx = int(((eeg_start_idx-initial_start_idx) / 500) * 8) # ex) (16000-15000)/500 *8 = 16

        hb_sample = fnirs_mat['Hb'][fnirs_start_idx:fnirs_start_idx+(fnirs_sampling_rate*global_slice_sec), :] # (40, 6)
        sliced_fnirs_data['Hb'].append(hb_sample)
        hbo_sample = fnirs_mat['HbO'][fnirs_start_idx:fnirs_start_idx+(fnirs_sampling_rate*global_slice_sec), :] # (40, 6)
        sliced_fnirs_data['HbO'].append(hbo_sample)
        thb_sample = fnirs_mat['THb'][fnirs_start_idx:fnirs_start_idx+(fnirs_sampling_rate*global_slice_sec), :] # (40, 6)
...
def trigger_named_trigger(act_name, trig_file, eeg_mat, fnirs_mat):
    ...
    # print(len(sliced_fnirs_data['THb'])) # 25
    # print(sliced_fnirs_data['THb'][0].shape) # (40, 6)
    ...
    return sliced_eeg_data, sliced_fnirs_data, True # list, dict, bool
...
def make_sliced_data_pkl(pths, root_path, trig_list):
    ...
    with open('./pickles/eeg_fnirs_sliced_data.pkl', 'wb') as f:
        pickle.dump(data, f, pickle.HIGHEST_PROTOCOL)
        print('>> eeg_fnirs_sliced_data.pkl saved.')

    # load all data pickle
    with open('./pickles/eeg_fnirs_sliced_data.pkl', 'rb') as f:
