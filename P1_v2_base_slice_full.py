...
def trigger_named_eeg(act_name, trig_file, eeg_mat, fnirs_mat):
    ...
    if act_name == 'V': # substract just before 30 sec mean
        ...
        for idx in range(len(targ_idx)):
            ...
            # baseline correction of verbal
            if idx == 0:
                baseline_correction = np.mean(eeg_data[0:eeg_start_idx,:], axis=0) # (32,)
            else:
                baseline_correction = np.mean(eeg_data[eeg_start_idx-15000:eeg_start_idx,:], axis=0) # (32,)
            eeg_sample = eeg_sample - baseline_correction
        ...
        # eeg / fnirs full
        eeg_sample = eeg_data[initial_start_idx:initial_start_idx+(200*eeg_sampling_rate), :] # 200 sec

        sliced_eeg_data = eeg_sample
...
def make_sliced_data_pkl(pths, root_path, trig_list):
    ...
    save_file_name = 'eeg_full_fnirs_full_baselinecorrected'
