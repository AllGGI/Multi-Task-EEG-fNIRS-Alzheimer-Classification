...
#####################################################################################
####### Slice FULL from first target+nontarget idx & remove end 30 sec ##############
#####################################################################################
...
def trigger_named_eeg(act_name, trig_file, eeg_mat, fnirs_mat):
    ...
    '''
        Option 3)
        C1 - from first target ~ end-30sec(eeg) | 0 ~ -30sec(fnirs)
        C2 - from first target ~ end-30sec | 0 ~ -30sec(fnirs)
        N1 - from first target ~ end-30sec | 0 ~ -30sec(fnirs)
        N2 - from first target ~ end-30sec | 0 ~ -30sec(fnirs)
        V - from first target ~ end-30sec | 0 ~ -30sec(fnirs)
    '''
    eeg_start_idx = targ_nontarg_idx[0] # ex) 15000
    eeg_full_sample = eeg_mat['data'][eeg_start_idx:-(30*eeg_sampling_rate), :] # (112112, 32)

    # print(eeg_mat['data'][eeg_start_idx:, :].shape) # (127112, 32)
    # print(eeg_mat['data'][eeg_start_idx:-(30*eeg_sampling_rate), :].shape) # (112112, 32)

    # print(fnirs_mat['Hb'].shape) # (2299, 6)
    # print(fnirs_mat['Hb'][:-(30*fnirs_sampling_rate), :].shape) # (2059, 6)
    fnirs_full_sample_dict = {}
    fnirs_full_sample_dict['Hb'] = fnirs_mat['Hb'][:-(30*fnirs_sampling_rate), :]
    fnirs_full_sample_dict['HbO'] = fnirs_mat['HbO'][:-(30*fnirs_sampling_rate), :]
    fnirs_full_sample_dict['THb'] = fnirs_mat['THb'][:-(30*fnirs_sampling_rate), :]

    return eeg_full_sample, fnirs_full_sample_dict, True 
...
def trigger_named_trigger(act_name, trig_file, eeg_mat, fnirs_mat):
    ...
    # print(eeg_full_sample.shape[0]/500, fnirs_full_sample_dict['Hb'].shape[0]/8, fnirs_full_sample_dict['HbO'].shape[0]/8, fnirs_full_sample_dict['THb'].shape[0]/8)
    # print('*'*100)
...
def make_sliced_data_pkl(pths, root_path, trig_list):
        ...
        for fn in eeg_file_list: # Patients
            ...
            for f in fn: # 6 act
                ...
                if act_name == 'RO': # save without slice
                    ...
                    if trig_key == 'trigger':
                        sliced_eeg_data, fnirs_full_sample_dict, well_done_flag = trigger_named_trigger(act_name=act_name, trig_file=this_patient_trig_file, eeg_mat=eeg_act_file, fnirs_mat=fnirs_act_file)
                        # print(sliced_eeg_data.shape)
                        eeg_act_datalist[act_save_order[act_name]] = sliced_eeg_data
                    elif trig_key == 'eeg':
                        sliced_eeg_data, fnirs_full_sample_dict, well_done_flag = trigger_named_eeg(act_name=act_name, trig_file=this_patient_trig_file, eeg_mat=eeg_act_file, fnirs_mat=fnirs_act_file)
                        ...
                        # print(sliced_eeg_data.shape)
                        eeg_act_datalist[act_save_order[act_name]] = sliced_eeg_data
                        fnirs_act_datalist[act_save_order[act_name]] = fnirs_full_sample_dict
    ...
    # with open('./pickles/eeg_fnirs_full_sliced_data.pkl', 'wb') as f:
    #     pickle.dump(data, f, pickle.HIGHEST_PROTOCOL)
    #     print('>> eeg_fnirs_full_sliced_data.pkl saved.')

    # # load all data pickle
    # with open('./pickles/eeg_fnirs_full_sliced_data.pkl', 'rb') as f:
